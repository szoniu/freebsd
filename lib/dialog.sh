...
_extract_bundled_gum() {
    ...
    if ! tar xzf "${tarball}" -C "${GUM_CACHE_DIR}" \
        "gum_${GUM_VERSION}_Freebsd_x86_64/gum" 2>/dev/null; then
        return 1
    fi

    # Move binary from subdirectory to cache root
    mv "${GUM_CACHE_DIR}/gum_${GUM_VERSION}_Freebsd_x86_64/gum" \
       "${GUM_CACHE_DIR}/gum" 2>/dev/null || true
    rmdir "${GUM_CACHE_DIR}/gum_${GUM_VERSION}_Freebsd_x86_64" 2>/dev/null || true
