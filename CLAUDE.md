...
`preflight → bsdinstall → mount_target → gpu → desktop → device_quirks → laptop → extras → finalize`.
...
├── laptop.sh           — laptop_setup_apply ("laptop" phase): gated on BATTERY_DETECTED,
│                         powerd + Cx states, suspend (S3-only, probed on the LIVE system),
│                         backlight devfs.rules (group video), I2C touchpad (ig4+iichid +
│                         psm synaptics hint), ThinkPad acpi_ibm; reuses _gpu_kld_list_append
│                         and umpc.sh's _quirk_note_* helpers
...
docs/                   — DESIGN.md (the research brief / spec) + HANDOFF.md (status) +
                          LIVE-TEST.md (field runbook) + LIVE-USB-CHECKLIST.md (per-machine
                          live checklist) + DAILY-DRIVER-AUDIT.md (laptop candidate verdicts)
presets/                — example.conf
tests/                  — Standalone tests (test_config / test_hardware / test_checkpoint /
                          test_validate / test_bsdinstall; shellcheck.sh); live-hw-check.sh
                          (POSIX sh — runs on the bare live medium, do NOT introduce bashisms)
...
- **MT7922 / RZ616 WiFi is unsupported (GPD Pocket 4 BLOCKER).** The `mt76` driver is in-tree but
  ...
  USB-Ethernet bootstrap. Surface WiFi is chip-dependent (AX200/AX201 via `iwlwifi` —
  802.11 a/b/g/n/ac since 14.3, no ax/WiFi 6 yet; QCA6174 dead).
...
bash tests/test_bsdinstall.sh    # bsdinstall script generation (ZFS/UFS/GELI, injection safety)
...
- **`docs/LIVE-USB-CHECKLIST.md`** — per-machine live-USB checklist for the laptop candidates
  (read-only `tests/live-hw-check.sh` first, then the install run); complements LIVE-TEST.md.
- **`docs/DAILY-DRIVER-AUDIT.md`** — laptop daily-driver audit: candidate ranking + verdicts
  that drove the `laptop` phase (suspend/backlight/touchpad/moused policy).
